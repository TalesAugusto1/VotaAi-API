...
exports.processOptionImages = exports.processImage = exports.uploadOptionImages = exports.uploadSingle = void 0;
...
const uploadOptionImages = (req, res, next) => {
    ...
    console.log(`[UPLOAD] Setting up upload with fields: ${fields
        .map((f) => f.name)
        .join(", ")}`);
    ...
    uploadMiddleware(req, res, (err) => {
        ...
        if (req.files) {
            ...
            console.log(`[UPLOAD] Received files for fields: ${Object.keys(files).join(", ")}`);
...
exports.uploadOptionImages = uploadOptionImages;
...
const processImage = (req, res, next) => {
    try {
        ...
        console.log(`[PROCESS] Processing image: ${req.file.fieldname}, size: ${req.file.size} bytes`);
...
const processOptionImages = (req, res, next) => {
    try {
        if (!req.files || Object.keys(req.files).length === 0) {
            console.log("[PROCESS] No files to process in processOptionImages middleware");
            return next();
        }
        console.log(`[PROCESS] Processing option images, fields: ${Object.keys(req.files).join(", ")}`);
        // Parse options from the request body if they exist as a string
        if (typeof req.body.options === "string") {
            try {
                req.body.options = JSON.parse(req.body.options);
                console.log(`[PROCESS] Parsed options JSON successfully, found ${req.body.options.length} options`);
            }
            catch (error) {
...
exports.processOptionImages = processOptionImages;
